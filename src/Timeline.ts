...
/**
 * Prepend a sequence of tasks to timeline. If a task is running, it will be paused until new prepended tasks finish.
 *
 * Task are executed in order in the list.
 */
...
class TimeoutTask implements Task {
  ...
  toString() {
    return (this.started ? "s" : "-") + (this.finished ? "f" : "-") + " " + this.name + " " + this.timeout;
  }
}

export const timeoutTask = (callback: () => any, timeout: number, name?: string): Task => {
  return new TimeoutTask(callback, timeout, name);
};
